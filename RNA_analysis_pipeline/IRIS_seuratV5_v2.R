...
PATH_output <- paste0("/home/",userID,
                      ...
                      "/Experiments/RNA_analysis/IRIS/AML_experiments/results/AML_samples/KMT2Ar_v_NPM1_v_HD/harmony")
...
find_cluster_resolution(seurat.object, dims_use, resolutions, min.pct, logfc.threshold,
                        integration.name,
...
expID.colors <- rep(alpha("lightgray", 0.2), length(all_expIDs))
names(expID.colors) <- all_expIDs

expID.colors[kmt2ar.pb] <- "orange"
expID.colors[kmt2ar.bm] <- "red"
expID.colors[kmt2ar.hd.pb] <- "green"
expID.colors[npm1.pb] <- "cyan"
expID.colors[npm1.bm] <- "darkblue"
...
for(cohort in all_cohorts){
  ...
  centroids <- aggregate(cbind(umap_1, umap_2) ~ seurat_clusters, data = plot_df, FUN = mean)
...
for(exp_id in unique(meta_data$expID)){
  print(exp_id)
  path <- paste0(PATH_input_IRIS_imaging, "/", exp_id, "/metadata/example_seurat_metadata.csv")
  md <- meta_data[meta_data$expID == exp_id,]
  write.csv(md, file = path, row.names = FALSE)
}

write.csv(meta_data, file = paste0(PATH_output_tables, "/example_seurat_metadata.csv"), row.names = FALSE)
