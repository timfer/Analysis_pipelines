rename_clusters <- function(seurat.object, new.cluster.names, path){
  ...
  # Add clean labels as a new metadata column
  seurat.object@meta.data$clean_clusters <- gsub("_", " ", Idents(seurat.object))
  
  # Plot using the new metadata column
  p <- DimPlot(seurat.object, reduction = "umap", label.size = 5, label = FALSE, 
               group.by = "clean_clusters") +
    theme(
